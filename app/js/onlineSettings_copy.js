...
function timeFun(data, func, tip, typeModel, timee, showWaiting) {
  console.log('timefunData:'+JSON.stringify(data));
  // 对获取唯一码成功的判断
  var time = arguments[4] ? arguments[4] : 5000;
  var theunique_id = '';
  vm.currentData = data;
  if(typeof data === 'object') {
    theunique_id = data.unique_id || data.uniqueId;
    vm.the_unique_id = data.unique_id || data.uniqueId;
  } else {
    theunique_id = data;
    vm.the_unique_id = data;
  }
  vm.socketFun = func;
  vm.socketTip = tip;
    ...
    vm.ajaxTimer = setTimeout(function() {
      ...
      mui.ajax(apiUrl.doGetUniqueIdInfo, {
        data: {
          ...
          token: vm.token,
          identifyCode: vm.currentIdentifyCode,
          uniqueId: vm.the_unique_id
        },
        dataType: 'json',
        type: 'post',
        timeout: 10000,
        success: function(data) {
          console.log('hd数据：'+JSON.stringify(data));
          if (data.status === 0) {
            tip = tip || data.info;
            var _data = data.data;
    ...
    if (hd[i].hitSicData == 'GET:GNSS.BASE.STATUS') {
      if (hd[i].status == true ) {
        if(hd[i].value == 2) {
          ...
          if(vm.baseStartCount<3){
            setTimeout(function() {
              baseStartAgain();
            }, 3000);
            vm.baseStartCount+=1;
          }else{
            mui.toast('基准站设置超时，启动失败!');
            vm.baseStartCount=0;
          }
...
function checkRes() {
 ...
 function baseStartAgain() {
   console.log(2222);
   console.log('vm.the_unique_id:'+vm.the_unique_id);
  timeFun(vm.baseStartData, baseStart, '失败11111');
  // mui.ajax(url, {
  //   data: {
  //     user_name: vm.user_name,
  //     token: vm.token,
